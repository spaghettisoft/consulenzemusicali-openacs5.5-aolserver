...
set server                    "consulenzemusicali" 
set servername                "Consulenze Musicali"

set serverroot                "/var/lib/aolserver/${server}"


# Defined in /etc/aolserver4/conf.d/openacs.sh 
set httpport                  [exec grep "^AOL_PORT" /etc/aolserver4/conf.d/${server}.sh | sed s/AOL_PORT=//] 
set address                   [exec grep "^AOL_ADDRESS" /etc/aolserver4/conf.d/${server}.sh | sed s/AOL_ADDRESS=//]
#---------------------------------------------------------------------
...
set pidfile                    ${serverroot}/run/${server}.pid
...
if { $database eq "oracle" } {
    ...
    set db_password           ""
    set db_port               "5432"
    set db_user               [exec grep "^AOL_USER" /etc/aolserver4/conf.d/${server}.sh | sed s/AOL_USER=//]
}
